...
DAGGER_STUDENT_DIR = os.path.join(C.DATASET_DIR, "dagger_student")
FIELDS = ["image", "direction", "step", "steer", "steer_rad", "nn_steer",
          "cte_m", "speed_mph", "x", "y", "yaw"]


def load_student(name, w, h, device):
    m = StudentNet(h, w).to(device)
...
def drive_collect(world, vehicle, img_queue, model, device, w, h, direction, round_dir, max_steps):
    ...
    for step in range(max_steps):
        ...
        rows.append(dict(image=rel, direction=direction, step=step, steer=exp_steer,
...
def main():
    ...
    model = load_student(args.student, args.w, args.h, device)
        ...
        for r in range(args.rounds + 1):
            ...
            for d in ["eastbound", "westbound"]:
                drows, st = drive_collect(world, vehicle, img_queue, model, device,
                                          args.w, args.h, d, round_dir, args.max_steps)
                rows += drows
            ...
            print(f"  re-distilling -> {new}", flush=True)
            distill_student(args.w, args.h, new, epochs=args.epochs, quiet=True)
            model = load_student(new, args.w, args.h, device)
            current = new
    finally:
