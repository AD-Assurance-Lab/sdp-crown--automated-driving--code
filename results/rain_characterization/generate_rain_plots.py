...
def main():
    ...
    output_expansion_all = os.path.join(output_dir, "rain_epsilon_expansion_plot.png")
    output_comparison_grid = os.path.join(output_dir, "rain_comparison_grid.png")
    output_json_all = os.path.join(output_dir, "rain_epsilon_expansion_analysis_combined.json")
    ...
    for split in splits:
        ...
        if not os.path.exists(dist_root) or not os.path.exists(ref_root) or not os.path.exists(gt_root):
            continue
    ...
    print(f"Rain bounds: eps_c 5th = {pct_c_5:.4f}, eps_b 95th = {pct_b_95:.4f}")
    
    # -------------------------------------------------------------------------
    # PLOT 1: ALL RAIN IMAGES EXPANSION PLOT (with percentile lines)
    # -------------------------------------------------------------------------
    run_eps_c_min = []
    run_eps_b_max = []
    ...
    # Subplot 1: Epsilon C
    plt.subplot(2, 1, 1)
    plt.scatter(indices_comb, eps_c_comb, color='#3b82f6', alpha=0.5, s=15, label='All ACDC Rain Images (Road Masked)')
    ...
    plt.title('(a) Road-Masked Contrast Scaling Bound Expansion ($\epsilon_c$) - All ACDC Rain Images', fontsize=13, fontweight='bold')
    ...
    # Subplot 2: Epsilon B
    plt.subplot(2, 1, 2)
    plt.scatter(indices_comb, eps_b_comb, color='#3b82f6', alpha=0.5, s=15, label='All ACDC Rain Images (Road Masked)')
    ...
    plt.title('(b) Road-Masked Brightness Bias Bound Expansion ($\epsilon_b$) - All ACDC Rain Images', fontsize=13, fontweight='bold')
    plt.xlabel('Number of Images Aggregated (Sorted Chronologically by Split)', fontsize=11)
    ...
    plt.close()
    print(f"Saved Plot 1 (Rain Expansion) to: {output_expansion_all}")
    ...
    # -------------------------------------------------------------------------
    # PLOT 2: RAIN COMPARISON GRID (results/rain_characterization/rain_comparison_grid.png)
    # -------------------------------------------------------------------------
    ...
    plt.suptitle("ACDC Rain Threat Characterization: Spatial Road-Masked Qualitative & Quantitative Comparison", 
                 fontsize=15, fontweight='bold', y=0.98)
    
    plt.subplots_adjust(wspace=0.02, hspace=0.26)
    plt.savefig(output_comparison_grid, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"Saved Plot 2 (Rain Comparison Grid) to: {output_comparison_grid}")
